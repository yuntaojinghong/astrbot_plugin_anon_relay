...
class FakeEvent:
    def __init__(self, text, sender="u1", platform="qq", private=True, chain=None):
        self._text = text
        self._sender = sender
...
async def main():
    ...
    # 2. 未开启匿名模式：静默且拦截（stop_event + 阻止默认 LLM）
    ...
    await check("群消息不处理", rs == [] and ev.stopped is False and "qq:u1" not in p5.sessions)

    # 5. 未配置目标群时无法开启
    p6, _ = make_plugin(None)
    ev = FakeEvent("开启匿名模式")
    rs = await collect(p6.on_message(ev))
    await check("无目标群提示", len(rs) == 1 and "目标群号" in rs[0] and "qq:u1" not in p6.sessions)
    ...
    await check("会话建立", "qq:u1" in p7.sessions and p7.sessions["qq:u1"]["anon_id"] == "匿名者-001")
    ...
    # 9. 长文分段（max_msg_len=10，20 字 → 2 条，头部只在第一条）
    ...
    await check("分段头部唯一", getattr(ctx9.sent[0][1][0], "text", "").startswith("【匿名倾诉】")
                and getattr(ctx9.sent[1][1][0], "text", "") == long_text[10:])

    # 10. 关闭会话
    ev = FakeEvent("结束倾诉")
    rs = await collect(p7.on_message(ev))
    await check("关闭回复", len(rs) == 1 and "已关闭" in rs[0])
    await check("会话清除", "qq:u1" not in p7.sessions)
    ...
    await check("持久化恢复", "qq:uPersist" in p11.sessions and p11.sessions["qq:uPersist"]["anon_id"].startswith("匿名者-"))

    # 12. 会话超时
    p12, _ = make_plugin({**TARGET, "session_timeout_min": 1})
    await collect(p12.on_message(FakeEvent("开启匿名模式", sender="uT")))
    p12.sessions["qq:uT"]["last_active"] = 0
    rs = await collect(p12.on_message(FakeEvent("还在吗", sender="uT")))
    await check("超时结束", len(rs) == 1 and "超时" in rs[0] and "qq:uT" not in p12.sessions)

    # 13. 多群转述
    p13, ctx13 = make_plugin({"target_group_ids": "111,222"})
